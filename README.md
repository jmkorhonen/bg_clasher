...
real geography. Designed with
[Sapper Studios' Battlegroup Clash: Baltics](https://www.sapperstudio.com/battlegr)
...
1. Download `bg_clasher.html` (the latest version) or `bg_clasher.zip` for a
   package with an example scenario included.
2. Open it in any modern browser (Chrome, Firefox, Edge, Safari — desktop).
...
This is a **beta**. The core feature set is stable and I've tested it a bit,
but I need outside eyes on:
...
- **Scenarios I haven't tried** — larger battles, nested formations, heavy
  modifier use, mounting chains, scenarios crossing the dateline or polar
  regions.
- **Cross-browser behaviour** — I've mostly tested in Firefox.
- **Data safety** — I want to know if any operation silently loses state.
  Always export before experimenting.
- **Rules-system fit** — BG Clasher ships with *Battlegroup Clash* modifier
  defaults, but the modifier library is editable per scenario. Tell me if the
  vocabulary (counter levels, permission model, visibility flags) works for
...
- **Order Graphics** — master scale slider for all order-arrow metrics
  (bracket size, shaft offset, hook radius), plus per-order-type pixel line
  weight overrides. Essential for battalion-level maps covering tens of km.
  Both side colours and order graphics can be exported together as a reusable
  visual theme.
- **Optional role passwords** for Game Mode. Saved in JSON as cleartext;
...
- **APP-6-inspired order arrow styling** — different visual styles per order
  type: parallel-line shafts for formation moves, hollow bracket arrowheads
  for main attacks, ticks-with-start-bar for breach, hook for withdraw, etc.
- **Measure** and **Angle** tools with live snap; click on units or markers
  to use their exact position as a waypoint.
- **Drawing tools** — polyline, polygon, circle, arrow, text, symbol.
...
- **Multiple windows** support via the 🖥️ button: open a mirror window
  ...
  onto a second monitor. Mirrors are read-only; no drag, no edit, no End
  Turn. Uses browser BroadcastChannel — no server needed.

### Detection and roles
- **UMPIRE** sees everything. **BLUFOR** / **OPFOR** see only their own
  assets and whatever the umpire has marked as detected for their side.
- Three detection states per unit per side: **Unknown** (hidden), **Detected**
  (question-mark icon only), **Identified** (silhouette + numeric ID +
  fields flagged as visible to the opponent).
...
- Any unit can carry any other unit as a passenger. Passengers are hidden
  from the map while mounted and move with their carrier (including on undo).
  Dismount places them 250 m NW of the carrier, on top of the stack so
...
- **💾 Export Scenario** writes the full state (including all turn history)
  to a JSON file.
- **📂 Import Scenario** reads it back. Automatic migrations handle older
  file formats: modifier libraries are seeded if missing, detection states
  are renamed to match the current vocabulary, suspected-icon overrides are
  sanitised, side colours / order graphics / role passwords get sensible
  defaults.
- **Export OOB + Templates** — both sides' OOB, referenced unit templates,
  and modifier library. The matching importer offers a side-chooser for
  single-sided files so you can drop an OPFOR file into the BLUFOR slot if
  you want.
- **Per-side OOB export / import** — each OOB tab has its own Export/Import
  buttons that carry just that side's tree plus only its referenced
  templates.
- **Export / Import Graphics** — side colours and order graphics as a
  reusable visual theme.
- **Export / Import Templates Only** — unit / marker / symbol / dice
  templates plus the modifier library, without any scenario state.

## Suggested playtest workflow

To shake out the most useful feedback in a 1–2 hour session:

1. **Set up a scenario from scratch** (5 min): name it, set a start date,
   pick a turn duration. Note whether the defaults feel reasonable.
2. **Build an OOB** (15 min): add 1 formation per side (give them
   abbreviations like "BG A" / "BG B"), add 2–4 units per formation with
   designations. Try mounting one unit inside another via the Unit Template's
   Available Modifiers whitelist.
...
4. **Plot orders** (10 min): multi-segment moves, fixed start times, mixed
   ...
   designation + formation labels. If arrows feel too small on your map,
...
8. **Reload in a fresh tab**: import the saved scenario. Everything should
   restore identically.
9. **Break things**: try unusual orderings (dismount a unit that just
   mounted, withdraw a carrier with passengers, delete an OOB entry that has
   a deployed unit, switch roles mid-modal). Report anything that crashes,
   loses data, or leaves the UI in a confused state.

## Reporting issues

Open an issue at <https://github.com/jmkorhonen/bg_clasher/issues>, or send
me a message directly. When you hit a bug, please include:
...
Mirror windows use the browser's built-in `BroadcastChannel` API to sync
state between tabs / windows on the same origin — also entirely client-side,
no network traffic.
...
- `state.scenario` — metadata, icon scale, modifier library, side colours,
  order graphics, role passwords, suspected-icon overrides.
- `state.oob` — per-side trees of formations and units.
- `state.unitTemplates` / `markerTemplates` / `symbolTemplates` /
...
Everything in `state` round-trips through JSON cleanly, which is the basis
for both save/load and the Turn History snapshot/revert mechanism.
...
2. **Local icon folder** configured in Scenario → General → "Local Icon
   Folder". Expected filenames are of the type `SIDE_Type_Name_Echelon.svg`
   (e.g. `BLUFOR_Mechanised_Infantry_Company.svg`).
3. **GitHub fallback**: fetched from the upstream repo of APP-6 icons. A
   tiny embedded set of "suspected enemy" fallbacks ship in the file so
   suspected markers render even offline.

Many thanks to
[Spatial Illusions' Unit Generator](https://spatialillusions.com/unitgenerator/)
for an excellent tool for generating the SVG files!
...
- Units lacking a `modifiers` array get one so rendering code can iterate
  without null-guards.
- Role passwords, side colours, and order graphics defaults are added if
  the older save didn't have them.
...
DOM manipulation with template strings. This keeps the file small and
...
- **Firefox** 115+ (primary development target).
- **Chrome / Edge** (Chromium 100+).
- **Safari** 16+ on macOS.

Known to **not** work:

- Internet Explorer. (ES2017+ syntax, modern CSS, arrow functions
  everywhere.)
...
Running from `file://` works but some browsers restrict cross-origin
fetches in that mode, which can prevent GitHub icon fetches. Two
workarounds:
...
- **No rule automation**. Modifiers track state but don't apply any
  dice-roll modifiers or combat arithmetic for you — by design, to keep the
  tool rules-agnostic. You adjudicate off the chips.
- **No multi-user networked play**. Mirror windows let multiple views of
  the same game coexist on one machine, but there's no cross-network sync.
  Pass the scenario file between devices instead.
...
or via the
[issue tracker](https://github.com/jmkorhonen/bg_clasher/issues).
...
*BG Clasher is built and maintained by Janne. Not affiliated with the
Battlegroup Clash rules authors.*
